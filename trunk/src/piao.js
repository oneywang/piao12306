...
function initAutoCommitOrder() {
  ...
  // stop the auto commit process!
    ...
    $.ajax({
      ...
      success: function (result) {
        if (result.op_2) {
          setCurOperationInfo(true, "排队人数（" + result.count + "）过多，铁道部拒绝排队，正在努力重试，如果长时间失败请重新订票或尝试手工提交！");
  ...
  function getSecondInfo(second) {
	...
	$("#rand").after(
    $("<span style='padding: 0 10px; position: absolute; margin-left: 230px; margin-top: 25px; line-height: 30px;'></span>")
    .append(
      $("<a href='#' style='padding: 4px 6px;font-size: 125%; background: #2CC03E;border-color: #259A33;border-right-color: #2CC03E;border-bottom-color:#2CC03E;color: white;border-radius: 5px;text-shadow: -1px -1px 0 rgba(0, 0, 0, 0.2);'/>")
      .attr("id", "btnAutoSubmit")
      .html("自动提交")
      .click(function () {
	      if (g_is_submitting) {
    ...
    .append(
      $("<span>").html("&nbsp;(帮您自动确认订单并在允许的情况下自动查询订单是否排队成功）")
    ));
  ...
  //输入完即提交
	$(".table_qr tr:last").before("<tr><td colspan='9'><label><input type='checkbox' id='autoStartCommit' /> 输入验证码后立刻开始自动提交，如果希望手工提交，请不要勾选。</label></td></tr>");
  ...
  //#region 自动选择联系人
	...
	$(window).ajaxComplete(function (e, xhr, s) {
	  if (s.url.indexOf("otsweb/order/confirmPassengerAction.do?method=getpassengerJson") != -1) {
	    tryFillPassage();
	  }
	});
